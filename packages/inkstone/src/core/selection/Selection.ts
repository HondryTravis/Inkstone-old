...
const NativeSelection = (editor: Editor) => {
    const { dom, utils, root } = editor
    console.log(dom)
    const { shadowRoot } = dom.root


    let extraRanges = [];

    shadowRoot.addEventListener('selectionchange', function (evt) {
        console.log('selectionchange',evt.target.getSelection())
    })
    ...
    const getActiveDocument = (rangeNode: Node) => {
        return (shadowRoot.nodeType === Node.DOCUMENT_FRAGMENT_NODE
            && shadowRoot)
    ...
    const setTagName = (element, newName) => {
        ...
        // 令替换元素成为在元素的 ownerDocument 上调用createElement（newname）的结果。
    ...
    const wrap = (nodeList, siblingCriteria, newParentInstructions) => {
        ...
        if (utils.isEditable(newParent.nextSibling)
            ...
            // “尽管新 parent 的 nextSibling 有 child，但将其第一个 child 追加为 new parent 的最后一个 child，以保留范围
